...
css(".noflow", { position: "relative", overflow: "hidden" })

$.warp = dom({ class: "noflow", style: { height: "100%", display: "flex" } }, shadow)
$.edtdiv = repleditor(data), $.execdiv = dom({ class: "stripe-background noflow" }, warp)
style(execdiv, { display: "flex", flexDirection: "column" })
...
const resizebar_width = 18, rbw = resizebar_width, pct = s => s * 100 + "%"
...
$.rszedt = (s = edtscale, left = `calc(${pct(s)} - ${rbw / 2}px)`) => (
  $.edtscale = s, style(edtdiv, { width: pct(s) }), style(allrb, { left }),
  style(edtrb, { left }), style(execdiv, { width: pct(1 - s) }))
...
$.rszexe = (s = exescale, top = `calc(${pct(s)} - ${rbw / 2}px)`) => (
  $.exescale = s, style(appdiv, { height: pct(s) }), style(allrb, { top }),
  style(exerb, { top }), style(dbgdiv, { height: pct(1 - s) }))
...
$.allrb = dom({ class: "resizer" }), edtrb.after(allrb)
style(allrb, { width: rbw, height: rbw, cursor: "row-resize", zIndex: "99" })
